...
void kernel_reconstruct( 
  ...
  float sum_amplitudes_input_adc = 0.;
  SampleVector amplitudes_input_adc;
  for (int iSample = 0; iSample<10; iSample++) {
    amplitudes_input_adc[iSample] = vector_vector_digis[ idx*10 + iSample ];
    sum_amplitudes_input_adc += vector_vector_digis[ idx*10 + iSample ];
  }
  
  if (sum_amplitudes_input_adc != 0.) { 
    ...
    status = multifit_fitting_NNLS.DoFit(amplitudes_input_adc,noisecov,activeBX,fullpulse,fullpulsecov,gainsPedestal,badSamples);
    ...
    if (status) {
      for (unsigned int iPulse=0; iPulse<multifit_fitting_NNLS.BXs().rows(); iPulse++) {
        ...
        vector_vector_amplitudes[ idx*10 + bx + 5 ] = multifit_fitting_NNLS.X().coeff(iPulse);
      }
    }
    else {
      for (unsigned int iPulse=0; iPulse<multifit_fitting_NNLS.BXs().rows(); iPulse++) {
